"""Pure-integer @njit kernels for PopOut board-state operations — no Python objects cross the JIT boundary.

Single Responsibility: this module owns only JIT-compiled board manipulation
functions (apply_move, legal_moves, is_full).
Mirrors the logic in src/engine/standard/bitboard.py but in Numba-compiled form.
